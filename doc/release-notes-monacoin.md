Dongri Core version *0.15.1* is now available from:

  <https://dongri.example.com/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/dongriproject/dongri/issues>
...
installer (on Windows) or just copy over `/Applications/Dongri-Qt` (on Mac)
or `dongrid`/`dongri-qt` (on Linux).
...
Dongri Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Dongri Core should also work on most other Unix-like systems but is not
...
A number of changes to the way Dongri Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 DNGR). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 DNGR) in the next release.
